...
@Autonomous(name="FTC_2018_SZ_FinalBot_Auto_Red1", group ="FTC 2018")

public class FTC_2018_SZ_FinalBot_Auto_Red1 extends LinearOpMode {
    ...
    public void Kick_Jewel() {
        data1 = robot.ColourSensorRed1.getLightDetected();
        data2 = robot.ColourSensorRed2.getLightDetected();
        if (robot.JewelRedLower < data2 && data2 < robot.JewelRedUpper){
            jewel = "Red";
        }
        else if (robot.JewelBlueUpper <= data2 && data2 <= robot.JewelRedLower) {//over-lapped data
            if (robot.JewelRedLower < data1 && data1 < robot.JewelRedUpper){
                jewel = "Red";
            }
            else if (robot.JewelBlueUpper <= data1 && data1 <= robot.JewelRedLower) {//over-lapped data
                jewel = "Over-lapped";
            }
            else if (robot.JewelBlueLower < data1 && data1 < robot.JewelBlueUpper){
                jewel = "Blue";
            }
        }
        else if (robot.JewelBlueLower < data2 && data2 < robot.JewelBlueUpper){
            jewel = "Blue";
        }

        telemetry.addData("Image",image);
        telemetry.addData("Jewel",jewel);
        telemetry.addData("SensorValue",data1);
        telemetry.addData("SensorValue",data2);
        telemetry.update();
